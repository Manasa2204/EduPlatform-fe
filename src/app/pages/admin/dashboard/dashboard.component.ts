...
@Component({ selector: 'app-admin-dashboard', templateUrl: './dashboard.component.html', styleUrls: ['./dashboard.component.scss'] })
export class AdminDashboardComponent implements OnInit {
  ...
  newCourse = {
    title: '', category: 'trending', tag: '', description: '', why: '', 
    price: 0, duration: '', certification: true, curriculum: [] as string[]
  };
  ...
  loadAll() {
    this.adminService.getApplications().subscribe(d => this.applications = d);
    this.adminService.getFaculty().subscribe(d => this.faculty = d);
    this.courseService.getCourses().subscribe(d => this.courses = d);
  }
  ...
  get groupedApplications() {
    ...
    this.applications.forEach(app => {
      if (!grouped[app.email]) {
        grouped[app.email] = {
          person: { name: app.name, email: app.email, phone: app.phone, linkedin: app.linkedin },
          applications: []
        };
      }
  ...
  onFacultySelect(event: any) {
    ...
    if (facultyMember && facultyMember.applications && facultyMember.applications.length > 0) {
      ...
      if (!this.newCourse.title) {
        ...
        this.curriculumInput = this.generateCurriculum(app.expertise, app.courseIdea).join('\\n');
      }
    }
  }
  ...
  addCourse() {
    const curriculum = this.curriculumInput.split('\\n').filter(item => item.trim());
    const courseData = {
      ...this.newCourse,
      curriculum,
  ...
  deleteFaculty(id: string, name: string) {
    if (confirm(`Are you sure you want to remove "${name}" from faculty? This will not delete their talent applications.`)) {
  ...
  resetForm() {
    this.newCourse = { title: '', category: 'trending', tag: '', description: '', why: '', price: 0, duration: '', certification: true, curriculum: [] as string[] };
    this.resetMentorData();
  ...
  extractCourseTitle(courseIdea: string): string {
    ...
    return words.join(' ') + (words.length < courseIdea.split(' ').length ? ' Course' : '');
  }

  suggestPrice(expertise: string): number {
    const prices: any = { 'java': 4999, 'python': 4499, 'devops': 5999, 'react': 3499, 'node': 3999 };
    const key = expertise.toLowerCase().split(',')[0].trim();
    return prices[key] || 3999;
  }

  suggestDuration(availability: string): string {
    const durations: any = { 'full-time': '8 weeks', 'part-time': '12 weeks', 'evenings': '16 weeks' };
  ...
  generateCurriculum(expertise: string, courseIdea: string): string[] {
    ...
    const base = ['Introduction & Basics', 'Core Concepts', 'Advanced Topics', 'Hands-on Projects', 'Best Practices', 'Certification Prep'];
    return base.map((item, i) => `${item}: ${topics[i % topics.length] || 'Practical Applications'}`);
  }
